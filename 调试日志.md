...
### 实车导航启动脚本
```bash
bash script/start_real_nav.sh --list-maps          # 列可选地图
bash script/start_real_nav.sh                      # 静态地图定位（默认 srm_site_01）
bash script/start_real_nav.sh --reloc              # 先验 PCD 重定位（small_gicp）
bash script/start_real_nav.sh --slam               # SLAM 建图
bash script/start_real_nav.sh --no-rviz --joy      # 关 RViz、开手柄
bash script/start_real_nav.sh --help
```
